  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!permissions || permissions.length === 0) {
        return res.status(403).json({ message: 'Access denied: No permissions assigned to role' });
      }
      ...
      res.status(500).json({ message: 'Error verifying permissions', error: error instanceof Error ? error.message : 'Unknown error' });
    }
  };
};